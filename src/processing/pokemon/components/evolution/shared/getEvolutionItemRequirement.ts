 ...
 */
export const GetEvolutionItemRequirement = (
  evolutionItemRequirement: string | number
): Identifyable => {
  let id: string;
  ...
  if (typeof evolutionItemRequirement === "number") {
    ...
    const foundItem = gameMaster.itemTemplate.find(
      (item) =>
        item.item &&
        item.item.itemId.toString() === evolutionItemRequirement.toString()
    );
