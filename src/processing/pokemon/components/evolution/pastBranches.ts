...
export class PastBranches implements IComponent {
  ...
  private GetRawPokemonById(pokemonId): ItemTemplate {
    return this.rawPokemons.find(
      (pokemon) => TemplateIdToId(pokemon) === pokemonId
    );
  }
  ...
  private GetPreviousRawEvolution(pokemonId: string): ItemTemplate {
    return this.rawPokemons.find((item) =>
      (item.pokemon.evolutionBranch || []).some(
        (evolution) => getPokemonIdByEvolutionBranch(evolution) === pokemonId
      )
    );
  }
   ...
   */
  private GetEvolutionCost(
    pokemonId: string,
    rawPokemon: ItemTemplate
  ): EvolutionCostToEvolve {
    if (!rawPokemon) return undefined;
    const evolutionBranch = rawPokemon.pokemon.evolutionBranch.find(
      (evolution) => getPokemonIdByEvolutionBranch(evolution) === pokemonId
    );
    ...
    if (evolutionBranch.evolutionItemRequirement) {
      evolutionItem = GetEvolutionItemRequirement(
        evolutionBranch.evolutionItemRequirement
      );
    }
    return {
  ...
  private GetPastBranch(pokemonId): PastEvolutionBranch {
    ...
    const rawPastPokemon = this.GetPreviousRawEvolution(
      TemplateIdToId(pastPokemon)
    );

    const costToEvolve = this.GetEvolutionCost(
      TemplateIdToId(pastPokemon),
      rawPastPokemon
    );
  ...
  Process(pokemons: Pokemon[], rawPokemons: ItemTemplate[]): Pokemon[] {
    ...
    return pokemons.map((pokemon) => {
      ...
      if (previousEvolution) {
        pokemon.evolution.costToEvolve = this.GetEvolutionCost(
          pokemon.id,
          previousEvolution
        );
      }
